...
```
1. 사용자가 회원가입 → 자동 테넌트 프로비저닝 (KafkaUser, Namespace 등 자동 생성)
2. 대시보드에서 PostgreSQL 또는 MariaDB Datasource 등록
   → CDC readiness 자동 검사 → 결과 표시
3. 캔버스에서 Datasource 노드 클릭 → "파이프라인 생성" 버튼
...
| 리소스 | 시점 | 누가 만듦 |
| --- | --- | --- |
| Namespace `tenant-{id}` | 회원가입 시 1회 | orchestrator |
| KafkaUser (consumer용) | 회원가입 시 1회 | orchestrator |
| Secret (KafkaUser 비번) | KafkaUser 생성 시 자동 | Strimzi |
| ResourceQuota | 회원가입 시 1회 | orchestrator |
| NetworkPolicy | 회원가입 시 1회 | orchestrator |

| 리소스 | 시점 | 누가 만듦 |
| --- | --- | --- |
| Secret (DB 자격증명) | Datasource 등록 시 | core (Datasource당 1개) |

| 리소스 | 시점 | 누가 만듦 |
| --- | --- | --- |
| KafkaConnector | Pipeline 생성 시 | orchestrator |
| KafkaTopic | Pipeline 생성 시 | orchestrator |

### 토픽 네이밍 + ACL 규칙

```
토픽 이름: tenant-{tenantId}.{schema}.{table}.cdc
예: tenant-abc123.public.orders.cdc

KafkaUser ACL:
- tenant-{tenantId}-consumer 사용자는
- topic prefix "tenant-{tenantId}." 에 대해서만 Read/Describe 권한
- 다른 테넌트 토픽은 접근 불가
```
...
| --- | --- | --- | --- |
| **core-service** | B | 8080 | ✅ |
| **orchestrator-service** | C | 8081 | ❌ 내부 |
| **ai-service** | D | 8082 | ❌ 내부 (Sprint 4에 합류) |
| **frontend** | E | 80 | ✅ |

### 2.2 통신 흐름 (대시보드 모드)

```
[Frontend] ─REST+WS─→ [core] ─REST─→ [orchestrator] ─K8s API─→ [K8s/Strimzi]
                          │
                          └─JDBC─→ [사용자 DB] (Inspector 모듈로 직접 연결)

[orchestrator] ─Kafka 이벤트→ [core] ─WS→ [Frontend]
```

자연어 모드 (Sprint 4 추가):
```
[Frontend] ─SSE─→ [core] ─SSE─→ [ai-service]
                                   │
                                   └─도구 호출─→ [core API] (이미 만들어진 거)
```
...
- 인증 (JWT 발급/검증)
- 회원가입 시 orchestrator에 테넌트 프로비저닝 요청
- Datasource CRUD (PG + MariaDB)
- Inspector 모듈 내장 (사용자 DB JDBC 검사)
- Pipeline CRUD (orchestrator에 위임)
- 자동 발견된 Service 관리
- WebSocket 실시간 푸시
...
| --- | --- |
| 1 | 회원가입/로그인, JWT |
| 2 | Datasource 등록 + Inspector (PG + MariaDB) |
| 2 | CDC readiness 검사 |
| 3 | Pipeline 생성/조회/삭제 |
| 3 | WebSocket 이벤트 푸시 |
| 3 | Kafka 이벤트 구독 (orchestrator로부터) |
| 3 | 자동 발견된 Service 조회 |
| 4 | 채팅 (ai-service에 위임) |
...
내부 흐름:
- MetaDB에 tenant + user 저장
- orchestrator에 프로비저닝 요청 (Namespace, KafkaUser, Quota, NetPol)
- 응답 대기 후 JWT 발급
- 프로비저닝 실패 시 롤백
```
...
내부 흐름:
- K8s Secret 생성 (tenant-{id}-ds-{datasourceId}-creds)
- MetaDB에 datasource 저장 (secret_ref 포함)
...
##### POST /api/datasources/{id}/recheck
```json
응답 202:
{
  "id": "uuid",
...
응답 202:
{
  ...
  "type": "CDC",
  "status": "PENDING",
  "sourceDatasource": { "id": "...", "name": "주문 DB", "dbType": "POSTGRESQL" },
  "tables": [...],
  "createdAt": "..."
}
...
- orchestrator에 위임 (POST /internal/pipelines)
- 즉시 202 반환
- 이후 status 변경은 WebSocket으로 push

검증:
- 같은 테넌트 내 name 중복 불가
- sourceDatasourceId의 cdcReadiness.status == GREEN 인지 확인
- tables가 비어있지 않은지
```
...
[
  {
    ...
    "sourceDatasource": {...},
    "tables": [...],
    "topicName": "tenant-abc.public.orders.cdc",
    "createdAt": "..."
  }
]
```
...
응답 200:
{
  ...
  "consumerConnectionInfo": {
    ...
    "topic": "tenant-abc.public.orders.cdc",
    "recommendedGroupId": "your-service-name",
    "security": {
      "protocol": "SASL_SSL",
      "mechanism": "SCRAM-SHA-512",
      "username": "tenant-abc-consumer",
      "passwordSecretName": "tenant-abc-consumer"
    },
    "codeSamples": {
      "java": "...",
      "python": "..."
...
[
  {
    ...
    "subscribedTopics": ["tenant-abc.public.orders.cdc"],
    "currentLag": 23,
    "status": "ACTIVE",
    "firstSeenAt": "...",
    "lastSeenAt": "..."
...
public class DatabaseInspectorFactory {
    public DatabaseInspector create(Datasource ds) {
...
{
  "namespace": "tenant-abc",
  "kafkaUser": "tenant-abc-consumer",
  "status": "PROVISIONED"
}

작업:
1. Namespace 생성
2. KafkaUser CRD 생성 (SCRAM + ACL: tenant-abc.* topics)
...
요청:
{
  ...
  "sourceDbSecretRef": "tenant-abc-ds-{datasourceId}-creds",
  "debeziumConfig": {
    ...
    "properties": {
      ...
      "database.password": "${secrets:tenant-abc-ds-xxx-creds/password}",
      "database.dbname": "orders",
      "plugin.name": "pgoutput",
      "slot.name": "debezium_pipeline_xxx",
      "publication.name": "dbz_pub_xxx",
      "table.include.list": "public.orders",
      "topic.prefix": "tenant-abc.public.orders"
    }
  }
}
...
- tenant prefix(`tenant-*-consumer`)로 테넌트 식별
...
| 도구 | 호출하는 core API |
| --- | --- |
| `list_datasources` | GET /api/datasources |
| `get_datasource_tables` | GET /api/datasources/{id}/tables |
| `inspect_datasource_readiness` | GET /api/datasources/{id} (cdcReadiness 필드) |
...
- 노드 타입:
  ...
  - Service (오른쪽)
- 상단 툴바: [+ Datasource 등록] [+ Pipeline 생성]
- 노드 클릭 → 우측 상세 패널
- (Sprint 4) 좌측 채팅 패널 토글

#### S3. Datasource 등록 위저드
...
#### S4. Pipeline 생성 위저드
- Step 1: 이름 입력
- Step 2: Source Datasource 선택 (드롭다운, CDC readiness = GREEN인 것만)
...
2. core가 MetaDB에 tenant + user 저장
3. core → orchestrator: POST /internal/tenants/provision
...
### 시나리오 2: Datasource 등록 (PG)

```
1. 사용자가 위저드에서 PG 정보 입력 + 비밀번호 + dbType = POSTGRESQL
2. Frontend → core: POST /api/datasources
3. core가 K8s Secret 생성 (tenant-abc-ds-{id}-creds)
4. core가 MetaDB에 datasource 저장 (status: CHECKING)
5. core가 201 즉시 응답
...
7. testConnection() → checkCDCReadiness()
   ...
   - max_wal_senders 확인
8. core가 MetaDB의 datasource 업데이트 (status: GREEN/YELLOW/RED + 상세 리포트)
9. core → WebSocket Frontend: DATASOURCE_INSPECTED
10. Frontend 캔버스에 Datasource 노드 등장 + readiness 색상 표시
```

### 시나리오 3: Datasource 등록 (MariaDB)
...
```
1. 사용자가 캔버스에서 Datasource 노드 클릭 → "파이프라인 생성"
2. 위저드: 이름 입력 → 테이블 선택 → 확인
3. Frontend → core: POST /api/pipelines
4. core 처리:
   a. Datasource 조회, cdcReadiness == GREEN 확인
   b. DatabaseInspectorFactory.create(datasource) → 적절한 Inspector
...
3. Kafka가 ACL 검증 (tenant-abc.* 토픽만 read 가능)
...
| B | core-service 스켈레톤, 인증 API, MetaDB Flyway |
| B | `DatabaseInspector` 인터페이스 + DTO 정의 PR |
| C | orchestrator-service 스켈레톤, fabric8 PoC |
...
| B | Datasource API (POST, GET, DELETE) + K8s Secret 생성 |
...
| D | (대기 또는 도구 정의 준비) |
| E | 로그인/회원가입 화면, Datasource 위저드 (실제 API 연동) |
| E | 캔버스 기본 (Datasource 노드 표시) |

**마일스톤**: PG/MariaDB Datasource 등록 → CDC readiness GREEN 확인 → 캔버스에 표시
...
- [ ] **PostgreSQL과 MariaDB 둘 다** Datasource 등록 + CDC 동작
- [ ] 사용자 DB의 INSERT/UPDATE/DELETE가 Kafka 토픽에 도착
- [ ] 컨슈머 연결 시 캔버스에 Service 노드 자동 등장
- [ ] 2개 이상의 테넌트 동시 사용해도 데이터/리소스 격리 (한 테넌트가 다른 토픽 못 봄)
...
4. **로깅 포맷**: JSON, 필수 필드 (traceId, tenantId, userId)
