...
대부분 턴은 전체가 아니라 mode·State 재사용으로 2~5단계만 탄다.
...
- `agent_run`(mode·status·incident_id) · `state_patch`(State 변경 이력) · `run_event`(SSE 재연결) · `approval_link`(Spring approval 캐시) · `report_snapshot`(검증된 final_response).
...
| Verifier 차단기 | `pass`만 Report로, `fail`/`needs_revision`은 책임 Agent로 되돌림 |
| 종료 보장 | 전역 step/토큰/시간 예산(`MAX_STEPS`=24) + 상한 가드를 Supervisor가 **중앙 집행** |
