...
- [§6 Failure Types](catalog-failure-types.md#6-catalog-failure-types)를 기준으로 incident type을 분류한다.
...
- [§8 Root Cause Catalog](catalog-root-causes.md#8-catalog-root-cause)에서 후보를 선택한다.
- [§9 Evidence Matrix](catalog-evidence-matrix.md#9-catalog-evidence-matrix)를 기준으로 required/supporting/negative evidence를 대조한다.
...
- [§11 Remediation Runbooks](catalog-remediation-runbooks.md#11-catalog-remediation-runbooks)를 기준으로 action 후보를 만든다.
...
- [§12 Policy Matrix](catalog-policy-matrix.md#12-catalog-policy-matrix)를 기준으로 risk와 decision을 정한다.
