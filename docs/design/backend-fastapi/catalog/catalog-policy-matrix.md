...
**최종 집행 기준은 Spring Boot의 `InternalOpsMutationController`, `MutationGate`, `PolicyGuard` 구현과 [governance.md §7 Mutation allowlist](../../backend-springboot/governance.md#7-mutation-allowlist)다.** 이 policy matrix는 사전 판단이며, 현재 Spring이 실제로 실행 가능한 mutation은 connector restart/pause/resume과 managed consumer group restart subset이다(불일치 시 Spring 기준 우선).
