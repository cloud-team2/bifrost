...
- 매 사용자 메시지마다 현재 router keyword 구현은 `simple_query`, `incident_analysis`, `action_execution` 중 하나를 선택한다. `approval_decision` enum/route는 남아 있지만 router 선택 branch는 없다.
- action keyword가 있으면 `action_execution`을 선택한다. 현재 router 구현은 `remediation_requested=false`를 항상 반환한다.
- `reuse_existing_analysis` 필드는 schema에 남아 있지만 현재 router 구현은 항상 `false`를 반환한다.
...
- 현재 Retrieval은 synthetic `store_ref`가 있는 `EvidenceItem` metadata를 만든다. raw evidence store 저장·hydrate는 아직 구현되어 있지 않다.
- redaction 상태를 기록한다.
...
- 현재 `run_verifier`는 executor output을 입력으로 받지 않으므로 실행 결과 검증은 수행하지 않는다. non-`simple_query`/`incident_analysis` mode는 기본 pass로 처리된다.
- Report 생성 전 RCA/실행 결과를 검증한다. 현재 `run_verifier`는 Report 본문을 입력으로 받지 않으므로 Report 내용 자체는 검증하지 않는다.
...
- 사용자에게 최종 응답을 작성한다.
- 현재 `run_report(user_message, retrieval_out, mode, llm)`은 retrieval evidence summary만 받아 plain answer를 만든다. root cause/confidence/action status는 Report 입력으로 전달되지 않는다. runner는 Verifier 미통과 상태여도 Report를 생성하고 snapshot `verified=false`로 표시한다.
