...
> 토픽 네이밍 규칙: **`cdc.table.{projectKey}.{dbSlug}.{schema}.{table}`** (table 중심). `dbSlug = {datasource.dbName}-{datasourceId 앞 8 hex}` — `name` 표시값이 아니라 물리 DB 이름(`db_name`)과 datasource 고유 id를 섞어 충돌을 막는다. Debezium `topic.prefix = cdc.table.{projectKey}.{dbSlug}` → `.{schema}.{table}` 자동 부여. KafkaUser ACL section은 현재 `TenantProvisioner`가 CR에 포함하지 않는다.
...
② 파이프라인 생성 시 (Spring Boot → Fabric8 → K8s API)
   KafkaConnector CR apply (Source = Debezium)
     → Debezium이 기동하며 토픽 자동 생성:
       cdc.table.{projectKey}.{dbSlug}.{schema}.{table}
...
- `topic.prefix = cdc.table.{projectKey}.{dbSlug}`(`dbSlug = {dbName}-{datasourceId 앞 8 hex}`, #265) → 토픽 `...{schema}.{table}` 자동 생성, `topic.creation.default.partitions=6`, `replication.factor=3`.
...
KafkaConnector source = new KafkaConnectorBuilder()
    ...
    .withNewSpec()
        ...
        .addToConfig("topic.prefix",      "cdc.table." + projectKey + "." + dbSlug)
