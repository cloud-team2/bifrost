...
> **Spring Boot가 집행하는 모든 mutation은 이 목록이 정본(SoT)이다.** FastAPI의 [tool catalog](../backend-fastapi/tool-catalog.md#4-tool-catalog)·[policy matrix](../backend-fastapi/catalog/catalog-policy-matrix.md#12-catalog-policy-matrix)는 이를 미러링한 *사전 판단*이며, 실행 직전 재검증([§3 신뢰 경계](#3-신뢰-경계))의 기준은 항상 이 표다. [api Part B](../../api/springboot.md)의 엔드포인트별 `정책` 열이 구현 상세이고, 런타임 조회는 `GET /internal/ops/admin/tool-catalog`다.
