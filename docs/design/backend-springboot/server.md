...
### 7.1 Operation Allowlist (집행 경계 단일 출처)

> **Spring Boot가 집행하는 모든 mutation은 이 목록이 정본(SoT)이다.** FastAPI의 [tool catalog](../backend-fastapi/tool-catalog.md#4-tool-catalog)·[policy matrix](../backend-fastapi/catalog/catalog-policy-matrix.md#12-catalog-policy-matrix)는 이를 미러링한 *사전 판단*이며, 실행 직전 재검증([§3 신뢰 경계](#3-신뢰-경계))의 기준은 항상 이 표다. Spring API의 현재 controller family는 [Controller Coverage](../../api/springboot.md#controller-coverage)에 정리한다. 런타임 조회 API `GET /internal/ops/admin/tool-catalog`는 설계 표면이며 현재 controller mapping은 WIP다.

**(1) 허용 operation × 게이트**

| 게이트 | operation |
| --- | --- |
| `require_approval` | connector task restart · connector restart · connector pause/resume · deployment scale · deployment rollout restart · pipeline pause/resume · rebalance proposal/approve/refresh · evidence tombstone |
| `require_change_management` | deployment rollback · pipeline backfill · pipeline rollback · topic create · topic config patch · KafkaUser create · KafkaUser ACL patch · connector config patch |
| `deny`(승인 무관) | pod exec/shell · 임의 SQL · secret 원문 read · topic delete · PVC/namespace delete · connector config 임의 overwrite · DB truncate · LLM 생성 manifest apply · consumer offset reset(v1 보류) · schema 강제 변경(v1 보류) |

read-only 조회(상태·지표·로그·trace·redacted config)는 자원 경계만 통과하면 `allow`다([§6](#6-read와-mutation-분리)).

**(2) operation과 무관하게 항상 적용하는 자원·실행 게이트**

| 게이트 | 내용 |
| --- | --- |
| caller | FastAPI service identity만 |
| 자원 범위 | project ownership·허용 namespace·**Bifrost 소유 워크로드**만. 고객 DB 내부·EKS 노드/클러스터 인프라는 집행 대상이 아니다(관측·escalation까지) |
| approval | approval id·action id·**params hash 일치**·미만료·single-use |
| change mgmt | change ticket·실행 window·rollback plan·impact analysis |
| idempotency | `X-Idempotency-Key` 필수, 중복은 replay |
| 기록 | 성공·실패·차단 모두 audit + before/after evidence |

이 둘(operation allowlist + 자원/실행 게이트)을 **모두** 통과한 요청만 runtime에 반영한다. 어느 하나라도 불명확하면 더 안전한 쪽으로(allow→approval→change→deny) 올리고 낮추지 않는다.
...
- **Prometheus·Loki·Tempo**: `monitoring` 네임스페이스의 **별도 스택**([infra §6.7](../infra.md#67-observability)). Spring은 HTTP client로 **질의만** 하고, 자신의 지표는 `/actuator/prometheus`로 노출해 scrape 대상이 된다.
