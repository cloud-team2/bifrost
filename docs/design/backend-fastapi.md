...
>
> **Confidence 임계 기준의 단일 출처는 §9 Evidence Matrix**다. (§1의 RCA 판단 표는 개념 요약이며 수치 기준은 §9를 따른다.)
...
| 범주 | 문서 |
| --- | --- |
| 장애 유형 | §6(Failure Types) |
| 장애 유형과 root cause 매핑 | §7(Incident→RootCause Map) |
| root cause 후보 | §8(Root Cause Catalog) |
| evidence 기준 | §9(Evidence Matrix) |
| alert 병합 | §10(Correlation Rules) |
| 대응 runbook | §11(Remediation Runbooks) |
| 정책/승인 기준 | §12(Policy Matrix) |
| Agent 역할 | §13(Agent Roles) |
| State schema | §14(State Schema) |
| workflow 제어 | §15(Workflow Control) |
| streaming event | §16(Streaming Events) |
| output schema | §17(Output Schemas) |
...
RCA Agent는 §8(Root Cause Catalog)에 정의된 후보만 선택한다. catalog에 없는 가능성은 `UNKNOWN_WITH_EVIDENCE_GAP`으로 보고한다.
...
세부 schema는 §14(State Schema)를 따른다.
...
원인별 required/supporting/negative evidence는 §9(Evidence Matrix)를 기준으로 한다.
...
자세한 분기 규칙은 §15(Workflow Control)에 둔다.
...
상세 병합 기준은 §10(Correlation Rules)를 따른다.
...
- Correlation Engine: 4.7 / §10(Correlation Rules)의 rule/score/window로 alert를 묶는다.
- Policy Guard: §12(Policy Matrix) lookup으로 `allow`/`require_approval`/`require_change_management`/`deny`를 결정한다.
...
역할별 책임과 금지 행위는 §13(Agent Roles)에 둔다.
...
이 순서는 "항상 전체를 실행한다"는 뜻이 아니다. Router는 매 사용자 메시지마다 mode를 재판정하고, 기존 run State가 유효하면 재사용해 필요한 단계만 실행한다. `incident_analysis`는 기본적으로 원인까지만 보고하고(`diagnose_only`), 조치 후보 생성과 실행은 사용자가 요청할 때만 진행한다. 단순 질의·승인 처리·조치 실행은 더 짧은 경로를 탄다. 의도별 최소 실행 단계는 §15(Workflow Control)를 따른다.

메인 workflow와 실패 시 되돌림 규칙은 §15(Workflow Control)를 기준으로 한다.
...
상세 schema와 patch 규칙은 §14(State Schema)를 따른다.
...
조치 후보는 §11(Remediation Runbooks)를 따르고, 위험도와 승인 기준은 §12(Policy Matrix)를 따른다.

Executor는 승인되었거나 변경관리 검증을 통과한 tool만 실행한다. 실행 가능한 tool 목록은 §4(Tool Catalog)에 둔다.
...
상세 event schema는 §16(Streaming Events)를 따른다.
...
2. 사전에 고정해야 하는 장애 유형, RCA 기준, runbook, policy, schema는 §6~§12(catalogs)와 §13~§17(contracts)에 둔다.
...
API 상세는 §3(API Reference), Agent 원리는 §1(Agent Principles), tool 매핑은 §4(Tool Catalog)를 기준으로 한다. Spring Boot 내부 운영 API는 [Spring Boot DETAILS](./backend-springboot.md)를 따른다.
...
상세 schema는 §14(State Schema)를 따른다.
...
상세 endpoint는 §3(API Reference)에 둔다.
...
- 각 tool은 §15의 개별 timeout/retry를 그대로 가진다. 한 tool이 timeout/실패해도 나머지 결과는 살리고, 실패는 evidence gap으로 RCA에 전달한다(부분 수집 허용).
...
| §2 Server Design | FastAPI Agent Server 설계 |
| §3 API Reference | Frontend-facing FastAPI API |
| §1 Agent Principles | Agent workflow와 판단 원칙 |
| §6~§12 Catalogs | 장애 유형, RCA 후보, evidence, runbook, policy |
| §13~§17 Contracts | Agent 역할, State, workflow, streaming, output schema |
| [Spring Boot DETAILS](./backend-springboot.md) | Operations Backend 설계 + 내부 운영 API |
| §4 Tool Catalog | Agent tool catalog와 mapping |
...
장애 유형은 “어디에서 문제가 관측되는가”를 기준으로 나눈다. RCA의 최종 원인 후보는 §8(Root Cause Catalog)를 따른다.

Classifier가 만든 `incident_type`이 어떤 `root_cause_id` 후보군으로 이어지는지는 §7(Incident→RootCause Map)를 기준으로 한다.
...
4. 모든 최종 root cause는 §8(Root Cause Catalog)와 §9(Evidence Matrix)를 만족해야 한다.
...
각 root cause는 §9(Evidence Matrix)의 필수 evidence 기준을 만족해야 확정 후보가 될 수 있다. 단, `UNKNOWN_WITH_EVIDENCE_GAP`, `MULTIPLE_POSSIBLE_CAUSES`, `CUSTOMER_OWNED_ROOT_CAUSE_LIKELY`는 확정 원인이 아니라 보류/에스컬레이션 상태이므로 별도 evidence matrix heading을 두지 않는다.
...
실행 가능 여부는 §12(Policy Matrix)와 Spring Boot Operations Backend가 최종 판단한다.
...
Runbook의 `Action`은 항상 Spring Boot mutation tool을 뜻하지 않는다. 추가 evidence 수집은 `workflow_action`, 고객사/플랫폼 전달은 `escalation`, 알림은 `notification`, 여러 조치 후보로 분해해야 하는 의도는 `composite_action`으로 둔다. 실제 실행 가능한 단일 tool은 §4(Tool Catalog)의 `runtime_tool` catalog에 등록되어야 한다.
...
Policy 변경은 audit와 replay test에 영향을 준다. 새로운 mutation tool을 추가할 때는 이 문서와 §4(Tool Catalog)를 함께 갱신한다.
...
Supervisor는 이 단계들을 제어하는 control layer다. Policy Guard와 Executor는 LLM 없이 동작하지만, 각자의 output 계약과 금지 행위(아래 §9, §10)는 그대로 적용된다.
...
- **plan의 독립적인 read-only tool은 병렬(fan-out)로 호출한다.** 서로 입력 의존이 없는 tool(예: `get_metrics`·`get_pipeline_logs`·`get_connector_status`)은 동시에 실행하고, 한 tool 결과가 다음 tool 입력이 되는 경우만 순차로 둔다. 순차 chain은 retrieval 단계 지연의 가장 큰 원인이므로 기본은 병렬이다(§4 Tool Catalog §13.1).
...
- §6(Failure Types)를 기준으로 incident type을 분류한다.
...
- §8(Root Cause Catalog)에서 후보를 선택한다.
- §9(Evidence Matrix)를 기준으로 required/supporting/negative evidence를 대조한다.
...
- §11(Remediation Runbooks)를 기준으로 action 후보를 만든다.
...
- §12(Policy Matrix)를 기준으로 risk와 decision을 정한다.
...
- 승인된 action만 §4(Tool Catalog)의 tool registry를 통해 실행한다.
...
| `correlation` | Correlation Engine | alert group, common root cause 후보 (Classifier는 hydration으로 읽기만, 쓰기 권한 없음 — §6) |
...
- Retrieval은 단계 안에서 **독립 read tool을 병렬 실행**해 retrieval wall-clock을 줄인다(§13.5, §4 Tool Catalog §13.1).
- 단계가 끝나는 대로 **부분 결과를 스트리밍**한다(§4.2, §16). 사용자는 전체 chain 완료를 기다리지 않고 진행 상황과 중간 RCA preview를 본다.
...
| 기법 | 적용 | 효과 |
| --- | --- | --- |
| **Retrieval 병렬 read tool** | 독립 read tool fan-out(§13.5, §4 §13.1) | retrieval wall-clock = Σtool → max(tool) |
| **부분 결과 스트리밍** | 단계 완료 즉시 event 전송, RCA 후보는 `report/preview`로 선노출(§16) | 체감 지연 ↓, 사용자는 최종 Report 전에 진행/중간 결론 확인 |
| **stage별 timeout** | 각 단계(특히 Retrieval/RCA/Verifier)에 개별 SLO timeout(§5.1) | 한 단계 지연이 run 전체를 잡지 않음 → 부분 결과로 Report |
...
| **모델 tier 분리** | Router/Planner/Classifier/Report=lightweight, RCA/Verifier=reasoning(§1 §10) | critical path에서 무거운 모델 호출 최소화 |
| **State 재사용** | §4.1 후속 turn 재사용 | 후속 turn 2~5단계 |

축약·병렬화는 **근거(evidence) 기반 판단과 Verifier 차단기 원칙을 우회하지 않는다.** Classifier+RCA를 합치더라도 §9 Evidence Matrix의 required evidence 검증과 confidence cap은 그대로 적용하고, 부분 결과 preview에는 "검증 전(preview)" 표시를 붙여 최종 Report(§13 Report, Verifier 통과분)와 구분한다.
...
workflow에는 순환 경로가 있다(`Verifier → 책임 Agent → … → Verifier`, `RCA evidence_gap → Planner → Retrieval → RCA`, `Policy Guard revise_action → Remediation → Policy Guard`, `Classifier scope_unclear → Planner → Classifier`). 단계별 retry만으로는 ping-pong 무한루프를 막을 수 없으므로, Supervisor는 **모든 run이 유한 단계 안에 Report로 수렴**하도록 다음 가드를 강제한다. 가드 카운터는 `run` namespace(§14)에 둔다.
...
- **새 evidence 없으면 루프 금지**: Retrieval이 직전 대비 새 `evidence_id`를 하나도 추가하지 못하면 그 가설은 "수집 가능한 근거 없음"으로 보고 재시도하지 않는다(Stop 조건 §10).
...
- **mutation 비재시도**: mutation timeout/실패는 자동 재실행하지 않는다(§4 Tool Catalog §15) — 실행 루프 자체가 생기지 않는다.
...
| --- | --- |
| `pass` | Report로 진행 |
| `fail` | Planner로 되돌림 (단, §5.1 `MAX_FAIL_LOOPS`까지). 상한 도달 시 Report로 진행 |
| `needs_revision` | 책임 Agent로 되돌림 (단, 같은 target revision은 §5.1 `MAX_REVISIONS`까지) |
...
- **§5.1 루프 가드 초과**(step/토큰/시간 예산, revision·evidence_gap·revise_action 상한)
...
if rca.status == "evidence_gap":
    if state.run.guards.gap_loops >= MAX_GAP_LOOPS or not has_new_evidence():
        finalize(report, reason="UNKNOWN_WITH_EVIDENCE_GAP")   # §5.1 루프 가드
    else:
...
if verifier.status == "needs_revision":
    ...
    if state.run.guards.revision_counts[t] >= MAX_REVISIONS:
        finalize(report, reason="revision_limit_reached")      # §5.1 루프 가드
    else:
...
if verifier.status == "fail":
    if state.run.guards.fail_loops >= MAX_FAIL_LOOPS:
        finalize(report, reason="verify_fail_limit_reached")   # §5.1 루프 가드
    else:
...
> 위 분기는 매 step 진입 시 **전역 가드**(`run.step_count`·LLM/token 예산·wall-clock)를 먼저 검사한다. 어느 하나라도 초과하면 즉시 `finalize(report, reason=...)`로 수렴한다(§5.1). 모든 `finalize`는 종료 사유를 timeline·audit에 남긴다.
...
| `report_preview_available` | Verifier 통과 전 중간 RCA preview 사용 가능(§4.2, `report/preview`). "검증 전" 표시 | yes |
