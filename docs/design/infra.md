...
구체적인 Kubernetes 리소스 목록과 현재 진행 상태는 [§2 Resource Plan](#2-리소스-계획현황-resource-plan)에 둔다. Spring Boot API와 Agent tool은 각각 [Spring Boot DETAILS](./backend-springboot.md), [FastAPI DETAILS](./backend-fastapi.md)를 기준으로 한다.
...
| Agent Run Store | `metadb` | **FastAPI 전용 PostgreSQL**(논리 DB `agentdb`) — run/state/event/approval/report. Spring metadb 테이블과 분리(서비스 경계, [fastapi §9](./backend-fastapi.md#2-server-design)) |
| Knowledge Vector Store | `metadb` | RAG 코퍼스 임베딩. v1은 `agentdb`에 **pgvector** 확장으로 co-locate(전용 벡터 DB 미추가), 확장 시 Qdrant/Milvus 등으로 외부화 |
