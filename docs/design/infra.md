...
| Agent Run Store | `metadb` | **FastAPI 전용 PostgreSQL**(논리 DB `agentdb`) — run/state/event/approval/report. Spring metadb 테이블과 분리(서비스 경계, [fastapi §9](./backend-fastapi/server-design.md#2-server-design)) |
