...
> **agentdb**: 별도 `agentdb` 네임스페이스 + 전용 pgvector 인스턴스. 인프라는 **빈 DB + pgvector 확장**까지 프로비저닝, 테이블 스키마(server-design §9.2: `agent_run`·`state_patch`·`run_event`·`approval_link`·`report_snapshot`)는 **앱 alembic 마이그레이션(FastAPI #134, initContainer 자동 적용 #255)** 소유. 매니페스트: gitops `databases/agentdb/` (ArgoCD `databases` 앱).
...
> **잔여**: ops-backend `search_logs`(Loki) 실연동 · Tempo OTLP 앱 계측(trace는 계측 전이라 비어있음, 다음 라운드).
>
