...
초기엔 `tenantdb`의 MariaDB/Postgres가 LoadBalancer로 노출돼 있었으나, 현재는 **ClusterIP로 전환되어 외부 미노출**이다. 동일 EKS 클러스터 내 접속 모델로 재정의:

- **네트워크 경로**: operations-backend(`bifrost-system`) → `tenant-postgres-service.tenantdb.svc.cluster.local:5432` / `tenant-mariadb-service.tenantdb.svc.cluster.local:3306` (in-cluster DNS, ClusterIP). 외부 노출 없음. *검증(2026-06-08): 서비스 엔드포인트 존재 + `bifrost-system`→`tenantdb` L4 도달 확인.*
...
> **외부 접속 주소**: `http://bifrost-app-alb-1898177525.ap-northeast-2.elb.amazonaws.com` (ALB, HTTP :80).
> **라우팅(실제 구조)**: 단일 Ingress(`frontend`, bifrost-system)가 **`/` → frontend**로 보내고, **`/api`·`/ws`는 frontend의 nginx가 operations-backend로 프록시**한다(별도 ALB path 규칙이 아니라 nginx `location /api { proxy_pass ... }`). ai-service는 내부(ClusterIP)만. operations-backend는 `operations-backend-secrets`(`JWT_SECRET`·`META_DB_PASSWORD`) 필요 — `META_DB_PASSWORD`는 metadb-credentials와 일치(#122/CD 부트스트랩 시 동기화).
