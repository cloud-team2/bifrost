...
> **외부 접속 주소 (#232 ALB→NLB 전환)**: `https://bifrost.skala-ai.com` — **단일 NLB → ingress-nginx → cert-manager(Let's Encrypt, HTTP-01) TLS**. HTTP→HTTPS 자동 리다이렉트. jenkins/harbor/argocd도 동일 NLB 경유 서브도메인(`https://{jenkins,harbor,argocd}.skala-ai.com`). 구 ALB+ACM은 폐기(ALB IAM ModifyListener 제약 회피 + 인증서 자동 갱신, ACM 잔여 인증서 삭제 완료). Route53 zone `skala-ai.com`에서 서브도메인 → NLB.
