...
FastAPI tool-catalog의 논리 read tool(`list_project_pipelines`·`get_pipeline_topology`·`get_connector_status`·`get_consumer_lag`·`search_logs`·`get_incident_summary`)과 이 엔드포인트의 입출력·error mapping·evidence 반환 매핑 계약은 [internal-ops-read-tools.md](./internal-ops-read-tools.md)에 정리되어 있다.

> 각 엔드포인트의 `정책` 열(read/approval/change management)은 [Spring server.md §7.1 Operation Allowlist](../design/backend-springboot/server.md#71-operation-allowlist-집행-경계-단일-출처)의 집행 경계를 구현하는 상세다. mutation 허용 집합과 거부(§26)의 **정본은 server.md §7.1**이며 이 문서는 그 구현·표면이다.
...
> **Approval source of truth = Spring Boot.** approval record(상태·params hash·승인자·만료·single-use)의 원본은 이 테이블이다. FastAPI의 `/api/v1/approvals/**`는 프론트용 facade로, 이 내부 API를 호출하고 run 연계 메타데이터(run_id↔approval_id, UI 표시용)만 보관한다. 승인 결정·검증·감사는 모두 Spring Boot가 집행한다([FastAPI Server Design §9 Persistence](../design/backend-fastapi/server-design.md#2-server-design)). 동일 approval을 양쪽에 중복 생성하지 않는다.
...
| Method | Path | 설명 | 정책 |
| --- | --- | --- | --- |
| `GET` | `/internal/ops/admin/tool-catalog` | Spring Boot에 등록된 operation catalog(=§7.1 집행 allowlist의 런타임 표현)를 조회한다. | admin |
