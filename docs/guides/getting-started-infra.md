...
> **플랫폼(operations-backend)이 프로비저닝하는 토픽**은 워크스페이스 격리를 위해 `{root}.{projectKey}.{dbSlug}.{schema}.{table}` 규칙을 따른다(`root=cdc.table|eda.table`, `dbSlug={dbName}-{datasourceId 앞 8 hex}`; [ADR 0002](../adr/0002-multi-tenancy-model.md), [design provisioning](../design/backend-springboot/provisioning.md#2-provisioning)). 위 단순 prefix는 로컬 검증용이다.
