...
3. **토픽이 자동 생성**된다: `cdc.table.{projectKey}.{dbName}.{schema}.{table}`.
...
> 토픽명은 **패턴과 무관**하게 `cdc.table.…` 접두사를 쓴다(과거 명명 잔재). EDA/CDC 모두 동일.
...
kubectl -n platform-kafka exec platform-kafka-kafka-0 -- \
  bin/kafka-topics.sh --bootstrap-server localhost:9092 --list | grep "cdc.table."
...
kubectl -n platform-kafka exec platform-kafka-kafka-0 -- \
  ...
  --topic cdc.table.<projectKey>.testdb.public.orders --from-beginning --max-messages 1 --timeout-ms 10000
...
토픽 `cdc.table.{PROJECT_KEY}.{SRC_DB}.{SRC_SCHEMA}.{SRC_TABLE}` 존재 확인 → `EDA E2E GREEN`.
...
- [ ] 토픽 `cdc.table.{projectKey}.{dbName}.{schema}.{table}` 자동 생성
