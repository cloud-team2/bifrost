...
[ADR 0001](./0001-polyrepo.md)은 7개 폴리레포 + 서비스별 MSA(core / orchestrator / ai 분리)를 결정했다. 그러나 실제 작업은 단일 레포(`bifrost`) 안에서 진행되었고, 설계 문서([design/overview.md](../README.md) §2, [springboot/DETAILS.md](../design/backend-springboot/server.md#5-패키지-구조) §5)는 백엔드를 **두 개의 백엔드**로만 정의한다.
...
- 설계: [design/backend/springboot/DETAILS.md](../design/backend-springboot/server.md#5-패키지-구조) §1, §5
