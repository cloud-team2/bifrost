...
[ADR 0001](./0001-polyrepo.md)은 7개 폴리레포 + 서비스별 MSA(core / orchestrator / ai 분리)를 결정했다. 그러나 실제 작업은 단일 레포(`bifrost`) 안에서 진행되었고, 설계 문서([design/overview.md](../README.md) §2, [springboot/DETAILS.md](../design/backend-springboot.md) §5)는 백엔드를 **두 개의 백엔드**로만 정의한다.
...
2. **Spring Boot는 단일 모놀리스** `services/operations-backend`로 통합한다.
   ...
   - 패키지 구조는 설계 [springboot/DETAILS.md §5](../design/backend-springboot.md)를 따른다.
...
- 설계: [design/backend/springboot/DETAILS.md](../design/backend-springboot.md) §1, §5
