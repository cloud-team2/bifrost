...
EVIDENCE_PROFILES: tuple[EvidenceProfile, ...] = (
    EvidenceProfile(
        ...
        required=(
            ...
            EvidenceRule(root_cause_id='SOURCE_DB_CONNECTION_TIMEOUT', kind='required', evidence='pipeline read latency 증가', example='read duration percentile 상승, extraction wait 증가'),
        ),
        supporting=(
    ...
    EvidenceProfile(
        ...
        required=(
            ...
            EvidenceRule(root_cause_id='SOURCE_READ_LATENCY', kind='required', evidence='extract task duration 증가', example='task runtime 증가, extract duration 증가, extract p95 상승, read stage duration 증가'),
        ),
        supporting=(
    ...
    EvidenceProfile(
        ...
        required=(
            EvidenceRule(root_cause_id='RECENT_CONFIG_CHANGE_REGRESSION', kind='required', evidence='config 변경 시점 이후 error/latency 증가', example='change time correlation, post-change error or latency signal', semantic_allowed=False, causality_type='temporal', temporality_required=True, causal_chain_step=1),
            EvidenceRule(root_cause_id='RECENT_CONFIG_CHANGE_REGRESSION', kind='required', evidence='변경 diff와 증상 계층이 연결됨', example='configuration diff affecting failing pipeline stage, validation failure after config update', semantic_allowed=False, causality_type='causal', causal_chain_step=2),
        ),
        supporting=(
    ...
    EvidenceProfile(
        ...
        required=(
            EvidenceRule(root_cause_id='RECENT_IMAGE_DEPLOYMENT_REGRESSION', kind='required', evidence='image rollout 이후 error/latency/restart 증가', example='deployment rollout event followed by restart or error-rate increase', semantic_allowed=False, causality_type='temporal', temporality_required=True, causal_chain_step=1),
    ...
    EvidenceProfile(
        ...
        required=(
            ...
            EvidenceRule(root_cause_id='PIPELINE_DUPLICATE_SPIKE', kind='required', evidence='retry/replay/backfill 또는 idempotency gap', example='repeated processing evidence, idempotency policy gap, replay/backfill boundary evidence'),
        ),
        supporting=(
...
def _flatten_evidence_profiles(
    profiles: Iterable[EvidenceProfile],
) -> tuple[EvidenceRule, ...]:
    return tuple(
        rule
        for profile in profiles
        for rule_group in (profile.required, profile.supporting, profile.negative, profile.exclusion)
        for rule in rule_group
    )


EVIDENCE_RULES: tuple[EvidenceRule, ...] = _flatten_evidence_profiles(EVIDENCE_PROFILES)
