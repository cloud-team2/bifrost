---
doc_id: ops_doc:monitoring
doc_type: ops_doc
title: Spring Boot Operations Backend — Monitoring & Incident Engine
tags: [ops, monitoring, incident-engine]
source: curated
---

# Spring Boot Operations Backend — Monitoring & Incident Engine

> 요약은 [overview.md](./overview.md). 이 파일은 **관측 데이터 수집 → 상태 산정 → 이벤트 기록/SSE 발행 → IncidentService 현재 연결 상태**와 모니터링 read(Sync/Messages/Metrics) 구현을 다룬다. 상태값·임계값·이벤트→인시던트 요구사항은 [기능명세서 부록 B](../../spec.md#부록-b--리소스-상태값-정의-및-자동-기준-단일-출처)에 모아 두고, 현재 구현은 코드 기준으로 확인한다.
>
...
상태 전이는 Watcher(event-driven), 지표는 주기 폴링, metric/log/trace는 on-demand 질의로 모은다. 주기·소스 요구사항은 부록 B.6 각 표에 모아 둔다.
...
- 폴링 결과는 전이가 있을 때 event row를 만든다. 일부 `KafkaAdminPoller`·`ConnectRestPoller` 경로는 `IncidentService.onThresholdViolation(...)`/`onRecovery(...)`로 인시던트에 연결하고, 일부 경로는 `EventService.record(...)`만 호출한다. poller가 SSE를 직접 발행하지는 않는다.
...
- **DB 상태의 파이프라인 전이(#179)**: `database`는 등록 1회가 아니라 `DatabaseHealthProbeJob`(60s)으로 주기 프로브하며, `UNREACHABLE`이 되면 `reevaluateForDatasource`로 해당 파이프라인을 `error`로 전이한다(원인 메시지 포함). connector가 RUNNING이어도 source/sink DB가 끊기면 파이프라인은 error다. 상세는 [lifecycle.md §2·§5](./lifecycle.md).
...
상태 전이·임계 초과를 `event` row로 적재한다. 카탈로그(트리거→레벨→인시던트 여부)의 요구사항은 [부록 B.6](../../spec.md#b6-이벤트-카탈로그)에 모아 둔다.
...
**[현재]** 인시던트 생성·에스컬레이션 동작(`IncidentService.java` 기준):

```text
IncidentService.onThresholdViolation(...):
  SloBurnRateService.decideIncident(...)로 severity/alert_route/severity_reason 산정
  기존 incident가 없고 route=DIAGNOSTIC이면 INFO event만 기록
  기존 활성(OPEN/INVESTIGATING) incident가 있으면:
     게이팅 없이 event를 attach하고, decision severity가 CRITICAL이면 escalation
  없으면:
     ERROR event → 즉시 incident 생성
     WARN event → 동일 grouping_key 30분 창에서 2건 도달 시에만 생성
...
- **severity/route** **[현재]**: `IncidentService`는 `SloBurnRateService.decideIncident(...)` 결과를 저장한다. user-impact SLI burn-rate 위반은 `PAGE`→`CRITICAL`, `TICKET`→`WARNING`으로 들어가고, 산정 근거는 `severity_reason`, 알림 경로는 `alert_route`에 저장된다(V25). SLO 측정값이 없으면 static threshold fallback(`ERROR`→`PAGE`/`CRITICAL`, `WARN`→`TICKET`/`WARNING`)을 쓰며, cause-only 신호가 사용자 영향 SLO 위반을 만들지 않으면 신규 인시던트 없이 diagnostic INFO event로 남긴다.
